...
from website.pydantic_ai_examples.config import Agent, logfire
...
async def post_chat(
    ...
    logfire.info("Prompt enriched with taxonomy")
    experiences = run_opensearch_query(
        question=prompt,
        index_name="experiences_4",
    ...
    prompt_template = Template(
        ...
        Use one sentence to summarize each high impact intervention and its outcomes.
        In parenthesis list the specific actions or products used, and the URL to the source.

        Biohacking results:
        ---------------------------------

        {% for biohack_type in taxonomy.biohack_types %}
...
if __name__ == "__main__":
    ...
    uvicorn.run(
        "website.pydantic_ai_examples.chat_app:app",
        reload=True,
